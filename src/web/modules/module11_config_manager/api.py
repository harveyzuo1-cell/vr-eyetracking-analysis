...
@m11_bp.before_request
def check_file_size():
    """检查上传文件大小"""
    if request.endpoint == 'module11.upload_background_image':
...
@m11_bp.route('/background-images/<task_id>', methods=['GET'])
...
@m11_bp.route('/roi/validate', methods=['POST'])
...
@m11_bp.route('/roi/generate-id', methods=['POST'])
def generate_roi_id():
    """
    生成下一个可用的ROI ID
...
@m11_bp.route('/health', methods=['GET'])
def health_check():
    """
    健康检查接口
